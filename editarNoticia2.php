	...
	<body>
		...
		<div class="wrapper">
	        ...
	        <div id="content-wrap">
	        	...
	        	<div class="page-title"><h1>Editar </h1><span>de areas de la JIM</span></div>
					...
					<div id="side-content">
						...
						$campus = $_POST['campus'];
						$query = pg_query("select * from noticias where id_noticia = $id_noticia");
						while($fila=pg_fetch_array($query)){
							$id_area = $fila['id_area'];
							$titular = $fila['titular'];
							$cuerpo = $fila['contenido'];
							$fecha = $fecha['fecha'];
						}
						$dia = "$fecha[0]$fecha[1]";
						$mes = "$fecha[3]$fecha[4]";
						...
						echo "<form action='guardareditNo2.php' method='post' >
								<table class='table-fill'>
								<th>Datos</th>
								<th></th>
								<tr>
									<input name='id_noticia' type='hidden' value='$id_noticia'>
									<td class='text-left'>Titular: </td>
									<td class='text-left'><input name='f_titular' type='text' value='$titular' ></td>
								</tr>
								<tr>
									<td class='text-left'>Cuerpo noticia: </td>
									<td class='text-left'><input name='f_cuerpo' type='text' value='$cuerpo' ></td>
								</tr>
									<td class='text-left'>Area: </td>
									<td class='text-left'><select name='f_area'>";
									while ($fila5 = pg_fetch_array($query5))
									{
										if ($fila5[0] == $id_area)
										{
										echo "<option selected='True' value=".$fila5[0].">".$fila5[2]."</option>";
										}
										else {
								...
								<tr>
									...
									<td class='text-left'><select name='f_dia'>
								...
								<tr>
									<td></td>
									<td class='text-left'><input  type='submit' value='Guardar' ></td>
								</tr>
						</table>
						</form>";
