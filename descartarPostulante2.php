...
if ($prioridad == 1)
{
	$query = pg_query("update postulantes set selec1 = 0 where id_dato = $id_dato");
	if (!$query)
	{
		...
		echo "<script language='javascript'>";
		echo "window.location='postulantes2.php'";
		echo "</script>";
	}
}
elseif ($prioridad == 2)
{
	$query = pg_query("update postulantes set selec2 = 0 where id_dato = $id_dato");
	if (!$query)
	{
		echo "<script language='javascript'>";
		echo "alert('Error al actualizar los datos')";
...
{
	$query = pg_query("update postulantes set selec3 = 0 where id_dato = $id_dato");
	if (!$query)
	{
		echo "<script language='javascript'>";
		echo "alert('Error al actualizar los datos')";
